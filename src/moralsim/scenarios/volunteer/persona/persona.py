...
from .cognition import (
    VolunteerActComponent,
    VolunteerDummyActComponent,
)
...
class VolunteerDummyPersona(MoralityDummyPersona):
    ...
    def __init__(
        ...
        self.act = VolunteerDummyActComponent(model, cfg, actions, max_num_rounds)
        self.act.init_persona_ref(self)


class VolunteerPersona(MoralityPersona):
    last_collected_resource_num: int
    other_personas: dict[str, "VolunteerPersona"]

    act: VolunteerActComponent
    ...
    def __init__(
        ...
        act_cls: type[VolunteerActComponent] = VolunteerActComponent,
    ) -> None:
        if scenario != "vd_base":
    ...
    def loop(self, obs: ActionObs) -> PersonaAction:
        ...
        if obs.current_location == "office" and obs.phase == "office":
            ...
            # One-shot game: skip reflection (no future round to inform).
